...
public class TileExpGen2 extends TileEntity implements IEnergySink, IInventory, IFluidHandler, IStandOn {
	...
	protected void transferExpToPlayer(EntityPlayer entityplayer) {
		 ...
		 if (fluid != null && fluid.isFluidEqual(new FluidStack(MainWV.FluidXP.xpJuice, 0))) {
			 if(fluid.amount > 0) {
				 do {
	...
	public void attemptGeneration() {
		...
		if(this.energy > 0) {
		this.fluidTank.fill(new FluidStack(MainWV.FluidXP.xpJuice, 3), true);
		this.energy -= 256;
		markDirty();
	...
	public boolean canFill(ForgeDirection from, Fluid fluid) {
		...
		return (fluid == MainWV.FluidXP.xpJuice);
	}
