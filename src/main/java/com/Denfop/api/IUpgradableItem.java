	...
	public enum EnumUpgrade {
			...
			public void onRemovedFromItem(ItemStack itemStack) {
				if (itemStack != null && itemStack.getItem() instanceof IEnergyContainerItem) {
					...
					for (int i = 0; i < 500
						...
						item.extractEnergy(itemStack,
								item.getEnergyStored(itemStack) - item.getMaxEnergyStored(itemStack), false);
				}
			}
		},
		DIG_SPEED(1, 1, "DigSpeed"), DIG_AOE(2, 4, "DigAOE") {
			public void onRemovedFromItem(ItemStack itemStack) {
				int profile = ItemNBTHelper.getInteger(itemStack, "ConfigProfile", 0);
				for (int i = 0; i < 8; i++) {
					ItemNBTHelper.setInteger(itemStack, "ConfigProfile", i);
					if (IConfigurableItem.ProfileHelper.getInteger(itemStack, "ToolDigAOE",
							0) > getUpgradePoints(itemStack))
						IConfigurableItem.ProfileHelper.setInteger(itemStack, "ToolDigAOE",
		...
		DIG_DEPTH(3, 2, "DigDepth") {
			public void onRemovedFromItem(ItemStack itemStack) {
				...
				for (int i = 0; i < 8; i++) {
					ItemNBTHelper.setInteger(itemStack, "ConfigProfile", i);
					if (IConfigurableItem.ProfileHelper.getInteger(itemStack, "ToolDigDepth",
							0) > getUpgradePoints(itemStack))
						IConfigurableItem.ProfileHelper.setInteger(itemStack, "ToolDigDepth",
								getUpgradePoints(itemStack));
				}
				ItemNBTHelper.setInteger(itemStack, "ConfigProfile", profile);
			}
		},
		ATTACK_DAMAGE(4, 1, "AttackDamage"), ATTACK_AOE(5, 2, "AttackAOE") {
			public void onRemovedFromItem(ItemStack itemStack) {
				int profile = ItemNBTHelper.getInteger(itemStack, "ConfigProfile", 0);
				for (int i = 0; i < 8; i++) {
					ItemNBTHelper.setInteger(itemStack, "ConfigProfile", i);
					if (IConfigurableItem.ProfileHelper.getInteger(itemStack, "WeaponAttackAOE",
							0) > getUpgradePoints(itemStack))
						IConfigurableItem.ProfileHelper.setInteger(itemStack, "WeaponAttackAOE",
								getUpgradePoints(itemStack));
				}
				ItemNBTHelper.setInteger(itemStack, "ConfigProfile", profile);
			}
		...
		ARROW_DAMAGE(6, 1, "ArrowDamage") {
			public void onRemovedFromItem(ItemStack itemStack) {
				...
				for (int i = 0; i < 8; i++) {
					ItemNBTHelper.setInteger(itemStack, "ConfigProfile", i);
					if (IConfigurableItem.ProfileHelper.getInteger(itemStack, "BowArrowDamage",
							0) > getUpgradePoints(itemStack))
						IConfigurableItem.ProfileHelper.setInteger(itemStack, "BowArrowDamage",
								getUpgradePoints(itemStack));
				}
				ItemNBTHelper.setInteger(itemStack, "ConfigProfile", profile);
			}
		},
		DRAW_SPEED(7, 2, "DrawSpeed"), ARROW_SPEED(8, 2, "ArrowSpeed") {
			public void onRemovedFromItem(ItemStack itemStack) {
				int profile = ItemNBTHelper.getInteger(itemStack, "ConfigProfile", 0);
				for (int i = 0; i < 5; i++) {
					ItemNBTHelper.setInteger(itemStack, "ConfigProfile", i);
					if (IConfigurableItem.ProfileHelper.getInteger(itemStack, "BowArrowSpeedModifier",
							0) > getUpgradePoints(itemStack))
						IConfigurableItem.ProfileHelper.setInteger(itemStack, "BowArrowSpeedModifier",
								getUpgradePoints(itemStack));
				}
				ItemNBTHelper.setInteger(itemStack, "ConfigProfile", profile);
			}
		},
		SHIELD_CAPACITY(9, 1, "ShieldCapacity"), SHIELD_RECOVERY(10, 1, "ShieldRecovery"),
		MOVE_SPEED(11, 1, "MoveSpeed"), JUMP_BOOST(12, 1, "JumpBoost");
		...
		public int getUpgradePoints(ItemStack itemStack) {
			...
			if (itemStack != null && itemStack.getItem() instanceof IUpgradableItem) {
				int points = ((IUpgradableItem) itemStack.getItem()).getBaseUpgradePoints(this.index)
						+ totalPoints / this.pointConversion;
				return Math.min(points,
						((IUpgradableItem) itemStack.getItem()).getMaxUpgradePoints(this.index, itemStack));
			}
			return 0;
		}
