...
public class TileEntityQuantumQuarry extends TileEntityElectricMachine
	...
	public List<ItemStack> list_ingot() {
		...
		if (Loader.isModLoaded("Forestry"))
			list.add(OreDictionary.getOres("gemApatite").get(0));
		if (OreDictionary.getOres("orePeridot").size() > 0)
			list.add(OreDictionary.getOres("gemPeridot").get(0));
		if (OreDictionary.getOres("oreMithril").size() > 0)
			list.add(OreDictionary.getOres("ingotMithril").get(0));
