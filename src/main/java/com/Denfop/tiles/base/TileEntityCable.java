...
public class TileEntityCable extends TileEntityBlock
		...
		if (nbttagcompound.hasKey("retextureRefMeta")) {
			...
			for (int i = 0; i < 6; i++) {
				if (nbttagcompound.hasKey("retextureRef" + i)) {
					this.retextureRef[i] = (Block) GameData.getBlockRegistry()
	...
	public void onNeighborBlockChange() {
		...
		for (Direction direction : Direction.directions) {
			...
			if (((neighbor instanceof IEnergyAcceptor && ((IEnergyAcceptor) neighbor)
					.acceptsEnergyFrom((TileEntity) this, direction.getInverse().toForgeDirection()))
					|| (neighbor instanceof IEnergyEmitter && ((IEnergyEmitter) neighbor)
							.emitsEnergyTo((TileEntity) this, direction.getInverse().toForgeDirection())))
				...
				if (neighbor instanceof TileEntityCable) {
					if (((TileEntityCable) neighbor).getCableThickness() < getCableThickness())
						...
						.isBlockSolid((IBlockAccess) this.worldObj, x, y, z, direction.getInverse().toSideValue())) {
					newRenderSide = (byte) (newRenderSide | mask);
				}
			}
			mask *= 2;
	...
	private void relight() {
		...
		Method propagateSkylightOcclusionMethod = ReflectionUtil.getMethod(Chunk.class,
				new String[] { "propagateSkylightOcclusion", "func_76595_e" }, new Class[] { int.class, int.class });
		...
		try {
			if (this.foamed == 2 && this.yCoord >= height) {
				relightMethod.invoke(chunk, new Object[] { Integer.valueOf(this.xCoord & 0xF),
						Integer.valueOf(this.yCoord + 1), Integer.valueOf(this.zCoord & 0xF) });
			} else if (this.yCoord == height - 1) {
				relightMethod.invoke(chunk, new Object[] { Integer.valueOf(this.xCoord & 0xF),
						Integer.valueOf(this.yCoord), Integer.valueOf(this.zCoord & 0xF) });
			}
			propagateSkylightOcclusionMethod.invoke(chunk,
					new Object[] { Integer.valueOf(this.xCoord & 0xF), Integer.valueOf(this.zCoord & 0xF) });
		} catch (Exception e) {
