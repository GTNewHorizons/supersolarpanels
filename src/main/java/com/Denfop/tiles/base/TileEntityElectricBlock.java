...
public int p;
  ...
  protected void updateEntityServer(InventoryPlayer inventoryplayer) {
    ...
    if (shouldEmitRedstone != this.isEmittingRedstone) {
     ...
     double sentPacket = 0.0;
    	...
    	if (this.chargeSlots[1] != null && this.chargeSlots[1].getItem() instanceof ic2.api.item.IElectricItem && this.energy > 0.0D) {
	        sentPacket = ElectricItem.manager.charge(this.chargeSlots[1], this.energy, 2147483647, false, false);
	        if (sentPacket > 0.0D) {
	        	needsInvUpdate = true; 
	        this.energy -= (int)sentPacket;}
	      } 
    	if (this.chargeSlots[2] != null && this.chargeSlots[2].getItem() instanceof ic2.api.item.IElectricItem && this.energy >= 0 && this.energy <= this.maxStorage) {

	        sentPacket = ElectricItem.manager.discharge(this.chargeSlots[2], this.energy, 2147483647, false, false, false);
