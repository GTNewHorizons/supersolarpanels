...
public class ModuleTypePanel extends Item implements IModulPanel {
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List info, boolean b) {
		...
		List<Double> list = IModulPanel.getData(itemStack);

		info.add(StatCollector.translateToLocal("supsolpans.SSP.GenerationDay.tooltip") + " " +ModUtils.getString( list.get(0)) + " EU/t ");
		info.add(StatCollector.translateToLocal("supsolpans.SSP.GenerationNight.tooltip") + " " + ModUtils.getString(list.get(1))
				+ " EU/t ");

		info.add(StatCollector.translateToLocal("ic2.item.tooltip.Output") + " " + ModUtils.getString(list.get(3)) + " EU/t ");
		info.add(StatCollector.translateToLocal("ic2.item.tooltip.Capacity") + " " + ModUtils.getString(list.get(2)) + " EU ");
		info.add(StatCollector.translateToLocal("ssp.tier") + ModUtils.getString(list.get(4)));
