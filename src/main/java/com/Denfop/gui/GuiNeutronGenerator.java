...
public class GuiNeutronGenerator extends GuiIC2 {
	...
	public GuiNeutronGenerator(ContainerNeutrniumGenerator container1) {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		this.fontRendererObj.drawString(((TileneutronGenerator) this.container.base).getProgressAsString(), 18, 31,
				4210752);

		super.drawGuiContainerForegroundLayer(par1, par2);
		FluidStack fluidstack = ((TileneutronGenerator) this.container.base).getFluidStackfromTank();
		if (fluidstack != null) {
			String tooltip = StatCollector.translateToLocal("ssp.fluidNeutron") + ": " + fluidstack.amount
	...
	protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
		...
		if (((TileneutronGenerator) this.container.base).getTankAmount() > 0) {
			IIcon fluidIcon = ((TileneutronGenerator) this.container.base).getFluidTank().getFluid().getFluid()
					.getIcon();
			if (fluidIcon != null) {
