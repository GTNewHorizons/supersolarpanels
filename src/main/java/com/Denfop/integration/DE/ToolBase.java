...
public class ToolBase extends RFItemBase {
	private static final Set SHOVEL_OVERRIDES = Sets
			.newHashSet(new Object[] { Blocks.grass, Blocks.dirt, Blocks.sand, Blocks.gravel, Blocks.snow_layer,
	...
	private static final Set PICKAXE_OVERRIDES = Sets.newHashSet(new Object[] { Blocks.cobblestone,
	...
	private static final Set AXE_OVERRIDES = Sets.newHashSet(new Object[] { Blocks.planks, Blocks.bookshelf, Blocks.log,
	...
	public List<ItemConfigField> getFields(ItemStack stack, int slot) {
		...
		if (!getToolClasses(stack).isEmpty())
			list.add((new ItemConfigField(4, slot, "ToolDigMultiplier"))
					.setMinMaxAndIncromente(Float.valueOf(0.0F), Float.valueOf(1.0F), Float.valueOf(0.01F))
					.readFromItem(stack, Float.valueOf(1.0F)).setModifier("PERCENT"));
		if (!getToolClasses(stack).isEmpty())
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		if (!world.isRemote && !BrandonsCore.proxy.isDedicatedServer()) {
			...
			DraconicEvolution.network.sendToServer(
					(IMessage) new ToolModePacket(InfoHelper.isShiftKeyDown(), InfoHelper.isCtrlKeyDown()));
		}
