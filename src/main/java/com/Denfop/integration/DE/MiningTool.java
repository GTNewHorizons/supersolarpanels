...
public abstract class MiningTool extends ToolBase implements IUpgradableItem {
					...
					for (int zPos = z - zMin; zPos <= z + zMax; zPos++) {
						if (player.worldObj.getTileEntity(i, yPos, zPos) != null) {
							if (player.worldObj.isRemote) {
								player.addChatComponentMessage((IChatComponent) new ChatComponentTranslation(
										"msg.de.baseSafeAOW.txt", new Object[0]));
							} else {
								((EntityPlayerMP) player).playerNetServerHandler.sendPacket(
										(Packet) new S23PacketBlockChange(x, y, z, ((EntityPlayerMP) player).worldObj));
							}
							return true;
						}
					}
		...
		for (EntityItem item : items) {
			if (!player.worldObj.isRemote) {
				...
				((EntityPlayerMP) player).playerNetServerHandler
						.sendPacket((Packet) new S18PacketEntityTeleport((Entity) item));
	...
	protected void breakExtraBlock(ItemStack stack, World world, int x, int y, int z, int totalSize,
		...
		if (!world.isRemote) {
			...
			if (event.isCanceled()) {
				((EntityPlayerMP) player).playerNetServerHandler
						.sendPacket((Packet) new S23PacketBlockChange(x, y, z, world));
				return;
			}
		}
		int scaledPower = this.energyPerOperation + totalSize * this.energyPerOperation / 10;
		if (player.capabilities.isCreativeMode
				|| (blockMap.containsKey(block) && ((Integer) blockMap.get(block)).intValue() == meta)) {
		...
		if (!world.isRemote) {
			...
			if (block.removedByPlayer(world, player, x, y, z, true)) {
				...
				if (block.getExpDrop((IBlockAccess) world, meta,
						EnchantmentHelper.getFortuneModifier((EntityLivingBase) player)) > 0)
					player.addExperience(block.getExpDrop((IBlockAccess) world, meta,
							EnchantmentHelper.getFortuneModifier((EntityLivingBase) player)));
			}
			...
			if (block.removedByPlayer(world, player, x, y, z, true))
				block.onBlockDestroyedByPlayer(world, x, y, z, meta);
			Minecraft.getMinecraft().getNetHandler().addToSendQueue((Packet) new C07PacketPlayerDigging(2, x, y, z,
