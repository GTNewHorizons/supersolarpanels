...
public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
    ...
    private void generateSurface(World world, Random random, int x, int y) { 
    	if(Config.EnableMickailOre)
    	 this.addOreSpawn(SuperSolarPanels.mikhail_ore, world, random, x, y, 16, 16, 3+random.nextInt(3), 14, 10, 60);
    	if(Config.EnableSpinelOre)
	        this.addOreSpawn(SuperSolarPanels.spinelore, world, random, x, y, 16, 16, 3+random.nextInt(3), 14, 10, 60);
    	if(Config.EnablePlatiumOre)
	        this.addOreSpawn(SuperSolarPanels.platiumore, world, random, x, y, 16, 16, 2+random.nextInt(3), 10, 0, 60);
    	if(Config.EnableWolframOre)
	        this.addOreSpawn(SuperSolarPanels.wolframore, world, random, x, y, 16, 16, 3+random.nextInt(3), 15, 10, 70);
    	if(Config.EnableChromiumOre)
	        this.addOreSpawn(SuperSolarPanels.chromiumore, world, random, x, y, 16, 16, 3+random.nextInt(3), 14, 10, 70);
    	if(Config.EnableIridiumOre)
	        this.addOreSpawn(SuperSolarPanels.iridiumore, world, random, x, y, 16, 16, 3+random.nextInt(3), 10, 10, 60);
    	if(Config.EnableMagnesiumOre)
	        this.addOreSpawn(SuperSolarPanels.magnesiumore, world, random, x, y, 16, 16, 3+random.nextInt(3), 12, 10, 70);
    	if(Config.EnableNickelOre)
	        this.addOreSpawn(SuperSolarPanels.nicelore, world, random, x, y, 16, 16, 3+random.nextInt(3), 12, 10, 60);
    	if(Config.EnableToriyOre)
	        this.addOreSpawn(SuperSolarPanels.toriyore, world, random, x, y, 16, 16, 3+random.nextInt(2), 10, 10, 70);
    	if(Config.EnableMagnetitOre)
	        this.addOreSpawn(SuperSolarPanels.magnetitore, world, random, x, y, 16, 16, 3+random.nextInt(2), 10, 10, 70);
    }
