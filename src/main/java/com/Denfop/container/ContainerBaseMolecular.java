...
public class ContainerBaseMolecular<T extends TileEntityBaseMolecular> extends ContainerElectricMachine<T> {
  ...
  public ContainerBaseMolecular(EntityPlayer entityPlayer, T tileEntity1, int height, int dischargeX, int dischargeY, int inputX, int inputY, int outputX, int outputY, int upgradeX, int upgradeY) {
    ...
    if (((TileEntityBaseMolecular)tileEntity1).inputSlot != null)
      addSlotToContainer((Slot)new SlotInvSlot((InvSlot)((TileEntityBaseMolecular)tileEntity1).inputSlot, 0, 20, 27)); 
    if (((TileEntityBaseMolecular)tileEntity1).outputSlot != null)
      addSlotToContainer((Slot)new SlotInvSlot((InvSlot)((TileEntityBaseMolecular)tileEntity1).outputSlot, 0, 20, 68)); 
    
    for (int i = 0; i < 3; ++i) {
        for (int k = 0; k < 9; ++k) {
            this.addSlotToContainer(new Slot((IInventory)entityPlayer.inventory, k + i * 9 + 9, 18 + k * 21, 98 + i * 21));
        }
    }
    for (int j = 0; j < 9; ++j) {
