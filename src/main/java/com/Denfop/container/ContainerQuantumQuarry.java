	...
	public ContainerQuantumQuarry(EntityPlayer entityPlayer, TileEntityQuantumQuarry tileEntity1) {
		...
		if (((TileEntityQuantumQuarry) tileEntity1).outputSlot != null) {
			...
			for (int j = 0; j < 6; ++j) {
				...
				addSlotToContainer((Slot) new SlotInvSlot((InvSlot) ((TileEntityQuantumQuarry) tileEntity1).outputSlot,
						j, 30 + 18 * j, 6));
			}
			for (int j = 0; j < 6; ++j) {
			...
			addSlotToContainer((Slot) new SlotInvSlot((InvSlot) ((TileEntityQuantumQuarry) tileEntity1).inputslot, 0, 8,
					6 + 18 + 9));
		}
	}
